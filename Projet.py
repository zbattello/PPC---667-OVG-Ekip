...
def market (shared_memory,coeff):
    energy_Price = 0.5
    energy_In = 0.0
    energy_Out = 0.0
    long_term_coeff = 0.999
    max_thread = 3
    ...
    while c<=5:
        ...
        if (end-start >= 5) :
            nb_thread = 0
            temperature = float("{:.2f}".format(shared_memory[1]))
            natural_disast_s = float("{:.2f}".format(shared_memory[2]))
            natural_disast_h = float("{:.2f}".format(shared_memory[3]))
            temp2 = float("{:.2f}".format(1/temperature))
            ...
            energy_Price = energy_Price*long_term_coeff + (1/temperature)*coeff[0] + natural_disast_s*coeff[1] + natural_disast_h*coeff[2]  
...
if __name__ == "__main__":
    ...
    market_process = ms.Process(target=market, args=(shared_memory,coeff))
